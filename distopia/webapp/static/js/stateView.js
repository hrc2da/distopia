...
export class StateView {
  constructor(initData, metricFocus = "population", counties) {
    ...
    let max = 1;
    if (this.metricFocus == "population") {
      max = 3000000;
    }
  ...
  setBounds(MIN_X, MIN_Y, MAX_X, MAX_Y) {
    ...
    this.xScale = d3
      .scaleLinear()
      .domain([MIN_X, MAX_X])
      .range([padding, xExtents + padding]);
    this.yScale = d3
      .scaleLinear()
      .domain([MIN_Y, MAX_Y])
  ...
  paintHistograms(data) {
    ...
    for (var i = 0; i < this.histograms.length; i++) {
      this.histograms[i].update(
        data[i].data,
        data[i].labels,
        { colors: UI_CONSTANTS[this.metricFocus].colors },
        max
      );
    }
  }
    ...
    let radarData = [
      {
        ...
        axes: [
          {
            axis: "Population",
            value: this.rescale_standardized(-data[0])
          },
          {
            axis: "Voter Efficiency",
            value: this.rescale_standardized(-data[1])
          },
          {
            axis: "Compactness",
            value: this.rescale_standardized(data[2])
          }
        ]
      }
    ];
  ...
  update(data, metric, precalculated_stats, isReset = false) {
    ...
    districtData.forEach((district, i) => {
      let distX_min = 1000000,
        distX_max = 0,
        distY_min = 1000000,
        distY_max = 0;
      let scale = isReset
        ? UI_CONSTANTS["init"].scale
        : UI_CONSTANTS[this.metricFocus].scale;
      ...
      district.precincts.forEach(precinct => {
        ...
        if (distX_min > this.counties[precinct].x[0]) {
          distX_min = this.counties[precinct].x[0];
        }
        if (distX_max < this.counties[precinct].x[1]) {
          distX_max = this.counties[precinct].x[1];
        }
        if (distY_min > this.counties[precinct].y[0]) {
          distY_min = this.counties[precinct].y[0];
        }
        if (distY_max < this.counties[precinct].y[1]) {
          distY_max = this.counties[precinct].y[1];
        }
      });
    });
    ...
    key
      .append("rect")
      .attr("width", 6 * (key_height - 40))
      .attr("height", key_height - 40)
      .attr("x", 0)
      .attr("y", 10)
      .attr("fill", "none")
      .attr("stroke", "black")
    ...
    if (domain[1] <= 1) {
      key
        .append("text")
        .attr("x", (key_height - 40) / 2)
        .attr("y", key_height - 16)
        .text(
          parseInt(domain[0] * 100) +
            "% " +
            UI_CONSTANTS[this.metricFocus].label
        )
        .attr("text-anchor", "middle")
        .attr("alignment-baseline", "middle");
      key
        .append("text")
        .attr("x", 5.5 * (key_height - 40))
        .attr("y", key_height - 16)
        .text(
          parseInt(domain[domain.length - 1] * 100) +
            "% " +
            UI_CONSTANTS[this.metricFocus].label
        )
        .attr("text-anchor", "middle")
      ...
      key
        ...
        .attr("y", key_height - 16)
        .text(
          "$" + parseInt(domain[0]) + " " + UI_CONSTANTS[this.metricFocus].label
        )
        .attr("text-anchor", "middle")
        .attr("alignment-baseline", "middle");
      key
        ...
        .text(
          "$" +
            parseInt(domain[domain.length - 1]) +
            " " +
            UI_CONSTANTS[this.metricFocus].label
        )
        .attr("text-anchor", "middle")
    ...
    for (var i = 0; i <= 5; i++) {
      key
        .append("rect")
        .attr("height", key_height - 40)
        .attr("width", key_height - 40)
        .attr("x", i * (key_height - 40))
        .attr("y", 10)
        .attr("fill", () => {
          if (domain[1] == 0.55) {
            return scale([domain[0] + i * step, 1]);
          } else {
            return scale([domain[0] + i * step, domain[domain.length - 1]]);
          }
        });
    }
    ...
    key.attr(
      "transform",
      "translate(" + (key_width - 6 * (key_height - 40)) / 2 + ",0)"
    );
    ...
    boundaries.forEach(bounds => {
      ...
      bounds.forEach((pt, i) => {
        if (i % 2 == 0) {
          ...
          b[b.length - 1].push(this.yScale(pt));
        }
      });
