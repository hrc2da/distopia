...
// initializations
var distopia = new DistopiaInterface({
  initialView: "state",
...
export function updateState(newState) {
  if (newState.metricFocus && State.metricFocus != newState.metricFocus) {
    distopia.handleCommand({
      cmd: "focus_state",
      param: newState.metricFocus,
...
function initInteractive() {
  ...
  districtSelector.onchange = () =>
    updateState({ selectedDistrict: districtSelector.value });
  ...
  resetCentroidsButton.onclick = () => {
    Object.keys(State.centroids).forEach(key => removeCentroid(key));
    initState();
  };
...
function initTasksAndTimers() {
  ...
  let randomTask = () =>
    tasks.splice(Math.floor(Math.random() * tasks.length), 1);
  ...
  let setTaskTime = () => {
    let taskTime = setInterval(() => {
      if (window.currentTime == 0) {
        ...
        d3.select("#task_number").text(
          "Task " +
            State.currentTaskNumber +
            " of " +
            TASK_DESCRIPTIONS.length +
            " :"
        );
...
function addCentroid(e) {
  ...
  const endDrag = d => {
    ...
    if (
      Math.abs(d3.event.x - d.startX) < 2 &&
      Math.abs(d3.event.y - d.startY < 2)
    ) {
      // console.log("no drag");
      return;
    }
    d3.select(idSelector)
      .attr("x", d3.event.x)
      .attr("y", d3.event.y);
    State.centroids[id]["coordinates"] = [
